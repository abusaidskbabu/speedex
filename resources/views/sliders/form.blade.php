...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'sliders?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
				...
				{!! Form::hidden('id', $row['id']) !!}					
				           ...
				           >{{ $row['heading'] }}</textarea> 
									  ...
									  <div class="form-group row  " >
										...
										<div class="col-md-6">
										  <textarea name='sub_heading' rows='5' id='sub_heading' class='form-control form-control-sm '  
				           >{{ $row['sub_heading'] }}</textarea> 
										 </div> 
										 <div class="col-md-2">
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/sliders/") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('sliders/comboselect?filter=tb_status:id:status') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("sliders/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
